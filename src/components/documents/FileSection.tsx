    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4">
        {documents.map((document) => (
          ...
          >
            {/* Top Row: Checkbox, Star, Analytics */}
            ...
            {/* Middle Row: File Icon and Name */}
            <div className="flex items-center gap-2 mb-2">
              <FileText className="h-5 w-5 sm:h-6 sm:w-6 text-gray-500 flex-shrink-0" />
              <span className="font-medium truncate text-sm sm:text-base" title={document.name}>{document.name}</span>
            </div>
            {/* Info Section */}
